...
if ($task) {
    ...
    # one, registered to run as SYSTEM (install_service.ps1) so it can
    # start before login -- instead of erroring, so an empty result here
    # doesn't prove the task is actually missing. schtasks.exe is more
    # honest about *why* it found nothing: "Access is denied" means the
    # task exists but this non-elevated session can't read its state;
