...
echo ""
echo "The hotkey needs Accessibility permission (System Settings -> Privacy &"
echo "Security -> Accessibility) -- macOS usually won't prompt for this on its own"
echo "for a plain venv Python process. If F9 does nothing, run"
echo "'venv/bin/python3 tray_client_mac.py' directly to see the 'not trusted' error,"
echo "then enable python3 under Accessibility (it's often auto-listed there,"
echo "unchecked, after the first failed attempt) and relaunch. Microphone access"
echo "does prompt natively the first time recording is attempted."
